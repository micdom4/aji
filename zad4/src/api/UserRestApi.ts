...
export const userApi = {
    ...
    getByLogin: async (login: string): Promise<AxiosResponse<UserType>> => {
        return await apiInstance.get(`${user_api}/login/${login}`)
            .catch((error) => {
                if (axios.isAxiosError(error) && error.response) {
                    if (error.response.status === 404) {
                        throw new Error(errorMessages.notFound)
    ...
    searchByLogin: async (login: string): Promise<AxiosResponse<UserType[]>> => {
        return await apiInstance.get(`${user_api}/search/${login}`)
    },
    ...
    deactivate: async (id: string) => {
        return await apiInstance.put(`${user_api}/${id}/deactivate`)
            .catch((error) => {
                if (axios.isAxiosError(error) && error.response) {
                    if (error.response.status === 404) {
                        throw new Error(errorMessages.notFound)
    ...
    activate: async (id: string) => {
        return await apiInstance.put(`${user_api}/${id}/activate`)
            .catch((error) => {
                if (axios.isAxiosError(error) && error.response) {
                    if (error.response.status === 404) {
                        throw new Error(errorMessages.notFound)
